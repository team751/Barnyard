...
class MainScreen(Screen):
    ...
    _scanning_process = None
    _scanning_process_queue = None
    ...
    def _scanning_process_queue_checker(self):
        self._part_uid = self._scanning_process_queue.get()

        if self.screen_active and self._part_uid is not None:
            self.refresh_part_data()

    def scan_tag(self, instance=None, add_widgets=True):
        if self.tag_uid_extractor is None:
            self.tag_uid_extractor = TagUidExtractor(os.getcwd() +
                                                     "/libNFCWrapper.so")
            self.tag_uid_extractor.init_device()
        ...
        self._part_uid = None
        self._scanning_process_queue = multiprocessing.Queue()

        self._scanning_process = multiprocessing.Process(target=self._scan_process,
                                                         args=(self._scanning_process_queue,))

        _thread.start_new_thread(self._scanning_process_queue_checker, ())

        self._scanning_process.start()

    def search_tags(self, instance=None):
        self._scanning_process.terminate()

        self.tag_uid_extractor.destroy()
        self.tag_uid_extractor = None
